...
namespace Models
{
    ...
    /// 3. SatisfiesConditionIsMet(): 判断条件是否满足
    ///
    /// 使用示例：
    /// - OnEnemyAttackCondition：敌人攻击时触发
    /// - OnFireBallCondition：使用火球术时触发
    /// - OnTurnStartCondition：回合开始时触发
    /// </summary>
    public abstract class PerkCondition
    {
        ...
        ///
        /// 使用场景：
        /// - Before：用于在动作发生前阻止或修改动作（如护盾、闪避）
        /// - After：用于在动作发生后执行附加效果（如反击、吸血）
        /// </summary>
        [SerializeField]
        ...
        /// 订阅条件 - 注册对特定游戏动作的监听
        ///
        /// 当特定游戏动作被触发时，会调用 reaction 回调
        /// 需要与 UnSubscribeCondition 配对使用，避免内存泄漏
        /// </summary>
        ...
        ///
        /// 当特权被移除或不再需要监听时调用
        /// 必须使用与 SubscribeCondition 相同的 reaction 参数
        /// </summary>
